...
entropy_lower_thresh_no_bp = -3.45
entropy_lower_thresh_bp = -3.65
    ...
    while True:
        ...
        for _ in range(n_iters):
            ...
            if iters == n_iters - 1:
                ...
                converged = entropy_lower_thresh_bp <= (ll1 + ll2) <= entropy_upper_thresh
                if converged:
        ...
        # If breakpoint is None, then have checked all possible breakpoints
        if converged or breakpoint is None:
            break
    if DEBUG:
        print(time() - s)
...
def decode_no_bp(ciphertext: str) -> str:
    ...
    for _ in range(n_iters):
        ...
        u, ll = compute_acceptance(f, f_prime)

        if np.random.uniform(0, 1) <= u:
            f = f_prime

        converged = ll >= entropy_lower_thresh_no_bp
        if converged:
            break
